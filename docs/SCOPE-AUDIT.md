...
Proceed with the selected MailSchema / Mail Action Protocol architecture, but reopen specification and conformance completion. The useful proposition remains an agent understanding and requesting a service action offered in an email, with the service enforcing permission and reporting the outcome. The present artifacts do not yet prove that contract completely or consistently.

**Implementation update, 24 September 2026:** the local protocol work now closes the reference-boundary defects in F1–F4 and F6–F8: 25 executable cases pass, 14 requirements are mapped, 19 artifacts are digest-bound and all 11 focused readiness probes pass. The Structured Email fixture and Nitrosend MIME builder now use the partial-representation shape and designation. Public package releases still contain the earlier contract and are labelled accordingly; the new full-contract promotion gate is ready for future changed releases. The Nitrosend action service is implemented and locally tested but is not merged or deployed. The Internet-Draft source now requests Standards Track status, names a contact author, carries the reconciled semantics and renders in CI. This update does not turn local evidence into a deployment, independent-adoption or IETF-submission claim. The findings below retain the defects observed at the audit baseline.
...
- `npm run check`: passes Registry, profile, 25-case MAP, 11-probe readiness, 14-requirement conformance and Astro diagnostics.
...
- [`scripts/audit-map-readiness.mjs`](../scripts/audit-map-readiness.mjs): all 11 focused probes pass. [`audits/2026-09-24-readiness.json`](audits/2026-09-24-readiness.json) records the observations and source hashes. This is a local diagnostic, not a deployed conformance certificate.
