...
**Implementation update, 24 September 2026:** the local protocol work now closes the reference-boundary defects in F1–F4 and F6–F8: 29 executable cases pass, 16 requirements are mapped, 20 artifacts are digest-bound and all 13 focused readiness probes pass. The Structured Email fixture and Nitrosend MIME builder now use the partial-representation shape and designation. Public package releases still contain the earlier contract and are labelled accordingly; the new full-contract promotion gate is ready for future changed releases. The Nitrosend action service is implemented and locally tested but is not merged or deployed. The Internet-Draft source now requests Standards Track status, names a contact author, carries the reconciled semantics and renders in CI. This update does not turn local evidence into a deployment, independent-adoption or IETF-submission claim. The findings below retain the defects observed at the audit baseline.
