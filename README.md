...
JavaScript, Python, Rust and Go packages are published from maintained language repositories. The exact releases currently recommended by the site come only from [`docs/releases/current.json`](docs/releases/current.json) and appear on [Tools](https://mailschema.org/tools/). Public readback verifies every artifact bundled in each selected release. The packages do not establish endpoint trust or grant authorization.
